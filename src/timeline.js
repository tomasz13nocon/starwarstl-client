...
const filterItem = (filters, item) => {
	...
	if (filters === undefined) console.error("This probably means a value from data not being present in filters (like type === 'whatever')");
	...
	if (dataLoaded) {
		...
		if (filterText) {
			...
			data = data.filter(item =>
				filterText.toLowerCase().split(" ").reduce((acc, value) =>
					acc && (item.title.toLowerCase().includes(value) || item.author?.toLowerCase().includes(value))
					, true)
			);
		}
		...
		<div className="container">
			{ dataLoaded ?
				<table id="timeline">
					...
					<tbody>
						{data.map((item) =>
							<TimelineRow key={item.id} item={item} activeColumns={activeColumns} expanded={expandedRow === item.id} />
						)}
					</tbody>
				</table>
				: <div>Loading...</div>}
