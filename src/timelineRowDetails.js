...
const imgAddress = filename => {
	...
	return `https://static.wikia.nocookie.net/starwars/images/${hash[0]}/${hash.slice(0, 2)}/${filename}`;
}
...
const getData = item => {
	...
	switch (item.type) {
		...
		case "comic":
			type = "Comic";
			switch (item.subtype) {
				case "Single issue": type += ", single issue"; break;
				case "Series": type += " series"; break;
				case "Story arc": type += ", story arc"; break;
				case "Trade paperback": type += " (TPB)"; break;
			}
			break;
		default:
			type = "Unknown";
	}
	type = <span className={"type-indicator " + item.type}>{type}</span>;
...
export default function TimelineRowDetails({ expanded = true, item, colspan }) {
	...
	React.useEffect(() => {
		if (detailsRef.current) {
			...
			detailsRef.current.style.height = expanded ? detailsRef.current.scrollHeight + "px" : 0;
		}
	}, [expanded]);
			...
			<tr className="details-row" ref={detailsRowRef}>
				<td colSpan={colspan}>
					<div
						className="details"
						ref={detailsRef}>
						<img /* height={260} */ width={200} src={imgAddress(item.cover)} className="cover" onLoad={imageLoaded} />
						<div className="text">
							<h3 className="title"><WookieeLink>{item.title}</WookieeLink></h3>
							<dl>
